...
let force = 200;
let VISCOSITY = 0.8 + sadness / 5;
let DENS_DECAY = 0.8 + boredom / 5;
...
function draw() {
  ...
  strongestEmotion = Object.entries(emotions).reduce((prev, curr) =>
  ...
  if (fury > 0.6) {
    if (random() < 0.9) {
      ...
      fluid.velocity[randx][randy].add(
        random() * force * 20,
        random() * force * 20
      );
...
class Fluid {
  ...
  clearDivergence() {
    ...
    for (let i = 0; i < this.w; ++i) {
      for (let j = 0; j < this.h; ++j) {
        this.velocity[i][j].x -=
          VISCOSITY * (p[wrap(i + 1, this.w)][j] - p[wrap(i - 1, this.w)][j]);
        this.velocity[i][j].y -=
          VISCOSITY * (p[i][wrap(j + 1, this.h)] - p[i][wrap(j - 1, this.h)]);
      }
    }
  }
    ...
    for (let i = 0; i < this.w; ++i) {
      for (let j = 0; j < this.h; ++j) {
        ...
        if (this.density[i][j] > 0.0) {
          ...
          let medianColor = constrain(
            mainPalette + this.density[i][j] * 25,
            mainPalette - RANGE,
            mainPalette + RANGE
          );
          let hue = map(
            medianColor,
            mainPalette + this.density[i][j] * 35,
            mainPalette - RANGE,
            mainPalette + RANGE,
            mainPalette - RANGE,
            mainPalette + RANGE
          ...
          fill(
            hue,
            SATURATION[strongestEmotion],
            LIGHTNESS[strongestEmotion],
            this.density[i][j] * 100
          );
    ...
    switch (strongestEmotion) {
      case "boredom":
        ...
        fluid.velocity[lx][ly]
          ...
          .setHeading(fluid.velocity[lx][ly].heading() + PI);
        fluid.velocity[rx][ry]
          .add(force * amtRX, force * amtRY)
          .setHeading(fluid.velocity[rx][ry].heading() + PI);
        break;
      ...
      case "excitement":
        ...
        fluid.velocity[wrap(lx - 1, SCALE)][ly]
          .add(force, force)
          .setHeading(2 * HALF_PI);
        fluid.velocity[lx][wrap(ly - 1, SCALE)]
          .add(force, force)
          .setHeading(3 * HALF_PI);
        fluid.velocity[wrap(lx + 1, SCALE)][ly]
          .add(force, force)
          .setHeading(4 * HALF_PI);
        fluid.velocity[lx][wrap(ly + 1, SCALE)]
          .add(force, force)
          .setHeading(5 * HALF_PI);

        fluid.velocity[wrap(rx - 1, SCALE)][ry]
          .add(force, force)
          .setHeading(2 * HALF_PI);
        fluid.velocity[rx][wrap(ry - 1, SCALE)]
          .add(force, force)
          .setHeading(3 * HALF_PI);
        fluid.velocity[wrap(rx + 1, SCALE)][ry]
          .add(force, force)
  ...
  if (mouseIsPressed === true) {
    ...
    switch (strongestEmotion) {
      case "boredom":
        ...
        fluid.velocity[x][y]
          .add(force * amtX, force * amtY)
          .setHeading(fluid.velocity[x][y].heading() + PI);
        break;
      ...
      case "excitement":
        ...
        fluid.velocity[wrap(x - 1, SCALE)][y]
          .add(force, force)
          .setHeading(2 * HALF_PI);
        fluid.velocity[x][wrap(y - 1, SCALE)]
          .add(force, force)
          .setHeading(3 * HALF_PI);
        fluid.velocity[wrap(x + 1, SCALE)][y]
          .add(force, force)
